...
public abstract class MenuEntry
{
    ...
    public abstract void Render(SpriteBatch spriteBatch, FontSystem font, Vector2 position, Color color, TextStyle textStyle, FontSystemEffect fontEffect, int fontEffectStrength);
}
...
public sealed class MenuButton : MenuEntry
{
    ...
    public override void Render(SpriteBatch spriteBatch, FontSystem font, Vector2 position, Color color, TextStyle textStyle, FontSystemEffect fontEffect, int fontEffectStrength)
    {
        var f = font.GetFont(FontSize);
        var length = f.MeasureString(Label);
        length.Y = FontSize;
        spriteBatch.DrawString(font.GetFont(FontSize), Label, position, color, origin: length * 0.5f, textStyle: textStyle, effect: fontEffect, effectAmount: fontEffectStrength);
    }
...
public sealed class MenuToggle : MenuEntry
{
    ...
    public override void Render(SpriteBatch spriteBatch, FontSystem font, Vector2 position, Color color,
        TextStyle textStyle, FontSystemEffect fontEffect, int fontEffectStrength)
    {
        string text = Label + ": " + IsChecked;
        var f = font.GetFont(FontSize);
        var length = f.MeasureString(text);
        length.Y = FontSize;
        spriteBatch.DrawString(font.GetFont(FontSize), text, position, color, origin: length * 0.5f, textStyle: textStyle, effect: fontEffect, effectAmount: fontEffectStrength);
    }
...
public sealed class MenuArraySelect<T> : MenuEntry
{
    ...
    public override void Render(SpriteBatch spriteBatch, FontSystem font, Vector2 position, Color color, TextStyle textStyle, FontSystemEffect fontEffect, int fontEffectStrength)
    {
        ...
        spriteBatch.DrawString(f, Label, position + _startingOffset, color, origin: new Vector2(0, FontSize) * 0.5f, textStyle: textStyle, effect: fontEffect, effectAmount: fontEffectStrength);
        spriteBatch.DrawString(f, "<", position + _arrowLeftOffset, color, origin: new Vector2(0, FontSize) * 0.5f, effect: fontEffect, effectAmount: fontEffectStrength);
        spriteBatch.DrawString(f, SelectOptions[CurrentOption].ToString(), position + _optionOffset, color, origin: new Vector2(currentOptionSize, FontSize) * 0.5f, effect: fontEffect, effectAmount: fontEffectStrength);
        spriteBatch.DrawString(f, ">", position + _arrowRightOffset, color, origin: new Vector2(0, FontSize) * 0.5f, effect: fontEffect, effectAmount: fontEffectStrength);
    }
...
public sealed class MenuTextInput : MenuEntry
{
    ...
    public override void Render(SpriteBatch spriteBatch, FontSystem font, Vector2 position, Color color, TextStyle textStyle, FontSystemEffect fontEffect, int fontEffectStrength)
    {
        ...
        var length = f.MeasureString(text);
        length.Y = FontSize;
        spriteBatch.DrawString(f, text, position, color, origin: length * 0.5f, textStyle: textStyle, effect: fontEffect, effectAmount: fontEffectStrength);
    }
